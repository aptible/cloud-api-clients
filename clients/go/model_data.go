...
func (dst *Data) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal JSON data into OperationFailure
	err = json.Unmarshal(data, &dst.OperationFailure);
	if err == nil {
		jsonOperationFailure, _ := json.Marshal(dst.OperationFailure)
		if string(jsonOperationFailure) == "{}" { // empty struct
			dst.OperationFailure = nil
		} else {
			return nil // data stored in dst.OperationFailure, return on the first match
		}
	} else {
		dst.OperationFailure = nil
	}
...
func (src *Data) MarshalJSON() ([]byte, error) {
	...
	if src.OperationFailure != nil {
		return json.Marshal(&src.OperationFailure)
	}
